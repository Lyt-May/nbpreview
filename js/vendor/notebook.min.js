(function()
{
    ...
    nb.Input.prototype.render=function(ci)
    {
        return this.work(ci,this.raw,this.cell)

        if(!this.raw.length)
        {
            var _ = makeElement("div")
            _.setAttribute("cell-id",ci);
            _.style.cssText = "position: relative;"
            return _
        }
        var holder=makeElement("div",["input"]);
        holder.setAttribute("cell-id",ci);
        ...
        var pre_el=makeElement("div");
        pre_el.setAttribute("cell-id",ci);
        pre_el.style.cssText = "position: relative;padding:5px;"
        ...
        var raw = this.raw.flat(Infinity)
        raw.forEach(function(line,l_id)
        {
            var label = makeElement("div")
            label.setAttribute("cell-id",ci);
            label.setAttribute("line-id",l_id);
            //label.style.cssText = "weight:100%;"
            label.id = "code-line-"+ci.toString()+'-'+l_id.toString()
            label.className = "code_line_label"
            label.style.cssText = "position: relative;"

            var bg_div = makeElement("div")
            bg_div.setAttribute("cell-id",ci);
            bg_div.setAttribute("line-id",l_id);


            var checkbox = makeElement("input");
            checkbox.setAttribute("type","checkbox");
            checkbox.setAttribute("name","select-line");
            checkbox.setAttribute("value",ci.toString()+"-"+l_id.toString());
            checkbox.setAttribute("cell-id",ci);
            checkbox.setAttribute("line-id",l_id);
            checkbox.setAttribute("onClick","return false;");
            checkbox.style.cssText = "position: relative;vertical-align:middle;margin-right:5px;"
            //div1.appendChild(checkbox);

            var div1 = makeElement("div")
            div1.setAttribute("cell-id",ci);
            div1.setAttribute("line-id",l_id);
            div1.style.cssText = "position: relative; display: inline-block; width:95%;vertical-align:middle; "
            //position: absolute;top: 0;bottom: 0;left: 0;right: 0;
            // /*
            var pre = makeElement("pre")
            pre.setAttribute("cell-id",ci);
            pre.setAttribute("line-id",l_id);
            pre.style.cssText = "padding: 0; overflow: visible;margin-left: 8px; margin-right: 8px;margin-bottom: 0; margin-top: 0;background-color: rgba(0,0,0,0);"

            // */

            var code_el=makeElement("code");
            code_el.setAttribute("cell-id",ci);
            code_el.setAttribute("line-id",l_id);
            code_el.setAttribute("data-language",lang);
            code_el.className="lang-"+lang;
            code_el.style.cssText = "background-color: rgba(0,0,0,0);" +
                "white-space: pre-wrap; " +
                "white-space: -moz-pre-wrap; " +
                "white-space: -pre-wrap; " +
                "white-space: -o-pre-wrap; " +
                "word-wrap: break-word;  "
            code_el.innerHTML=nb.highlighter(escapeHTML(line),pre_el,code_el,lang);

            /*
            var code_line = escapeHTML(line)
            var head = code_line.search(/[^\s]/)
            if(head==-1)
                code_el.innerHTML=nb.highlighter(code_line,pre_el,code_el,lang);
            else
            {
                code_el.innerHTML=nb.highlighter(code_line.slice(head),pre_el,code_el,lang);
                if(head!=0)
                {
                    var pre = makeElement("pre")
                    pre.setAttribute("cell-id",ci);
                    pre.setAttribute("line-id",l_id);
                    pre.innerHTML=code_line.slice(0,head)
            ...
            */
            ...
            div1.appendChild(pre)
            //div1.appendChild(code_el)

            /*
            var div2 = makeElement("div")
            div2.setAttribute("cell-id",ci);
            div2.setAttribute("line-id",l_id);
            div2.style.cssText = "position: absolute;top: 0;bottom: 0;left: 0;right: 0;width:100%;"
            //div2.style.height = div1.clientHeight.toString()+"px"
             */

            label.appendChild(checkbox);
            label.appendChild(div1);
            //label.appendChild(div2);
            //label.style.height = div1.clientHeight.toString()+"px"
            pre_el.appendChild(label);
            //console.log(div1.clientHeight)
            //console.log(line)

            // var linebreak = doc.createTextNode('\n')
            // pre_el.appendChild(linebreak);
        })
        /*
        var code_el=makeElement("code");
        code_el.setAttribute("cell-id",ci);
    ...
    nb.Input.prototype.work = function (ci,raw) {
    ...
    nb.Output.prototype.renderers={display_data:render_display_data,execute_result:render_display_data,pyout:render_display_data,pyerr:render_error,error:render_error,
        stream:function()
        {
            var el=makeElement("div",[this.raw.stream||this.raw.name]);
            var raw=joinText(this.raw.text);
            el.innerHTML=nb.highlighter(nb.ansi(escapeHTML(raw)),el);
            return el
        }
    };
    nb.Output.prototype.render=function(ci)
    {
        var outer=makeElement("div",["output"]);
        outer.setAttribute("cell-id",ci);
    ...
    nb.Cell.prototype.renderers={
        ...
        code:function(ci)
        {
            ...
            cell_el.appendChild(this.input.render(ci));

            var total_output=makeElement("div",["total-output"]);
            total_output.setAttribute("cell-id",ci);
            var output_els=this.outputs.forEach(function(o)
            {
                total_output.appendChild(o.render(ci))
                //cell_el.appendChild(o.render(ci))
            });
            cell_el.appendChild(total_output)
            return cell_el
        }
    };
    nb.Cell.prototype.render=function(ci)
    {
        ...
        var st_hr = makeElement("hr");
        st_hr.setAttribute("cell-id",ci);

        //var ed_hr = makeElement("hr");
        //ed_hr.setAttribute("cell-id",ci);

        if(this.type=='code')
        {
            st_hr.style.cssText = "width: 168%; left: -68%; position: absolute; top: -0.325em;"
            //ed_hr.style.cssText = "width: 240%; left: -70%; position: absolute; bottom: -0.325em;"
        }
        else
        {
            st_hr.style.cssText = "right: 0; left: 0; position: absolute; top: -0.325em;"
            //ed_hr.style.cssText = "right: 0; left: 0; position: absolute; bottom: -0.325em;"
        }
        el.appendChild(st_hr)
        //el.appendChild(ed_hr)
    ...
    nb.Worksheet=function(raw,notebook)
    {
        ...
        this.render=function()
        {
            ...
            worksheet_el.style.cssText = "position: relative;"
    ...
    nb.Notebook.prototype.render=function()
    {
        ...
        notebook_el.style.cssText = "position: relative;"
        this.worksheets.forEach(function(w){notebook_el.appendChild(w.render())});
        this.el=notebook_el;
        return notebook_el
    };
    nb.setInputRender = function(render_func)
    {
        nb.Input.prototype.work = render_func
    }
